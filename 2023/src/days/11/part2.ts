...
const GALAXY_EXTENSION = 1_000_000 - 1;

function manhattanGalactic(a: Point, b: Point, filledRows: Set<number>, filledCols: Set<number>) {
  const x = Math.abs(b.x - a.x);
  const xExtension = Array(x)
    .fill(0)
    .map((_, i) => (filledCols.has(Math.min(a.x, b.x) + i + 1) ? 0 : GALAXY_EXTENSION) as number)
    .reduce((sum, num) => sum + num, 0);
  const y = Math.abs(b.y - a.y);
  const yExtension = Array(y)
    .fill(0)
    .map((_, i) => (filledRows.has(Math.min(a.y, b.y) + i + 1) ? 0 : GALAXY_EXTENSION) as number)
    .reduce((sum, num) => sum + num, 0);
