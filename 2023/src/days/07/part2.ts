...
function getHandType(hand: string) {
  ...
  const jokerIndex = symbols.findIndex(({ symbol }) => symbol === 'J');

  if (jokerIndex >= 0) {
    const jokerSymbol = symbols.splice(jokerIndex, 1);
    jokerCount = jokerSymbol[0].count;
  }

  switch ((symbols[0]?.count ?? 0) + jokerCount) {
    case 5:
...
const hands = data.map(([hand, bid]) => ({ hand, bid: parseInt(bid), type: getHandType(hand) }));
