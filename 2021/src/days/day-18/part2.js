...
for(let i = 0; i < input.length; i++) {
	for(let j = 0; j < input.length; j++) {
		if(i === j) continue;
		const resultTree = sumTrees(arrayToTree(stringToArray(input[i])), arrayToTree(stringToArray(input[j])));
		const mag = computeMagnitude(resultTree);
		largestMagnitude = Math.max(largestMagnitude, mag);
	}
}

console.log(`Magnitude of the largest pair : ${largestMagnitude}`);